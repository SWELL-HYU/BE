...
COMMENT ON COLUMN user_item_view_logs.item_id IS '조회한 아이템 ID';
COMMENT ON COLUMN user_item_view_logs.view_started_at IS '조회 시작 시간';
COMMENT ON COLUMN user_item_view_logs.duration_seconds IS '머무른 시간 (초)';
CREATE INDEX idx_user_item ON user_item_view_logs(user_id, item_id);
